...
public class CMPP_SX implements CMPP_Service{
    ...
    private int initCMPP()throws Exception{
        ...
        try {
            ...
            if(stat!=CMPP_SUCCESS){
                Log.printEvent("初使化SX_CMPP发生错误,resp stat:"+stat,logFile);
            }else{
                Log.printEvent("初使化SX_CMPP成功"+Thread.currentThread().getName(),logFile);
            }
        }
        catch (Exception ex){
            stat=CONNECT_INIT_ERROR;
            Log.printError(ex,"初使化SX_CMPP发生错误"+Thread.currentThread().getName(),logFile);
        }
        try {
    ...
    private byte[] getAuthString(String time){//取得登录认证码（转换前）
        ...
        byte[] timeStamp = time.getBytes();//spm.getTimeStamp().getBytes();
    ...
    private int login(CMPPConnectMessage cMsg,byte[] auth,String time)throws IOException,Exception{
        ...
        length=12;
        System.arraycopy(spm.getUSER(),0,inMsg,length,spm.getUSER().length);
        length+=spm.getUSER().length;
        ...
        if(head.nCommandID==this.COMMAND_CMPP_CONNECT_RESP){
            ...
            Log.printEvent("SX:登陆连接反馈发生错误！",logFile);
    ...
    private boolean activeTest(String flag)throws IOException{
        ...
        if(flag==null||(int)(System.currentTimeMillis()/1000)-checkTime>5)
            testFlag=true;
        if(testFlag){
