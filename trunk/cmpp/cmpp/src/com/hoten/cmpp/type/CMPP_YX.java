...
public class CMPP_YX implements CMPP_Service{
    ...
    private int initCMPP()throws Exception{
        ...
        try {
            ...
            if(stat!=CMPP_SUCCESS){
                ...
                Log.printEvent("初使化YX_CMPP成功"+Thread.currentThread().getName(),logFile);
            }
        }
        catch (Exception ex){
            stat=CONNECT_INIT_ERROR;
            Log.printError(ex,"初使化YX_CMPP发生错误"+Thread.currentThread().getName(),logFile);
        }
        try {
    ...
    private byte[] getAuthString(){//取得登录认证码（转换前）
        ...
        System.arraycopy(abyte2, 0,auths, 0, k);//取得２５位的MD5加密前值
        return auths;
    }
